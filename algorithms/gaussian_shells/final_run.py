...
if type(first_proposal_index) is int:
    ...
    if first_proposal_index == 'good_original':
        ...
        if dim == 2:
            ...
            first_proposal = load_first_proposal(70)
        elif dim == 20:
            first_proposal = load_first_proposal(282)
        else:
            raise NotImplementedError('No such first proposal')

    elif first_proposal_index == 'good_variational':
        if dim == 2:
            first_proposal = load_first_proposal(479)
        elif dim == 10:
            first_proposal = load_first_proposal(434)
        elif dim == 20:
            first_proposal = load_first_proposal(485)
        else:
            raise NotImplementedError('No such first proposal')

    elif first_proposal_index == 'average_variational':
        if dim == 2:
            first_proposal = load_first_proposal(489)
        elif dim == 10:
            first_proposal = load_first_proposal(492)
        elif dim == 20:
            first_proposal = load_first_proposal(493)
        else:
            raise NotImplementedError('No such first proposal')
...
if method == "original":
    ...
    for i in range(max_sampling_steps):
        ...
        if i == 0:
            ...
            vb = pypmc.mix_adapt.variational.GaussianInference(weighted_samples[:,1:], initial_guess=sampler.proposal,
                                                               weights=weighted_samples[:,0]) #no posterior2prior here!
        vb.run(max_vb_steps, abs_tol=10**-5, rel_tol=10**-10, prune=mincount, verbose=True)
        sampler.proposal = vb.make_mixture()
