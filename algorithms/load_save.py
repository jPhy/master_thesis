...
def initialize_final():
    "Run this function before first use of ``save_final_proposal``."
    if os.path.exists('./final_proposals/database.info'):
        raise IOError('database exists')
    with open("./final_proposals/database.info", 'w') as dbfile:
        cPickle.dump([], dbfile)
...
def save_final_proposal(proposal, creation_parameters):
    """Save a ``proposal`` to "./final_proposals/i.prop" where i is the
    first free integer. Add attribute creation_parameters to ``proposal``.
    Save ``creation_parameters`` to "./final_proposals/database.info"
    ...
    # find out filename for proposal
    i = 0
    while os.path.exists('./final_proposals/'+ str(i) + '.prop'):
        i += 1
    propfilename = './final_proposals/'+ str(i) + '.prop'
    ...
    # update database
    with open("./final_proposals/database.info", 'r') as dbfile:
        db = cPickle.load(dbfile)
    db.append(creation_parameters)
    with open("./final_proposals/database.info", 'w') as dbfile:
        cPickle.dump(db, dbfile)
...
def load_final_proposal(i):
    """Load proposal number ``i`` saved by :py:func:`.save_final_proposal`.
    The proposal is read off the file "./final_proposals/i.prop"
    Can also parse negative ``i`` (standard python interpretation of
    negative numbers).
    ...
    if i < 0:
        # find out filename for proposal (standard python interpretation of negative numbers)
        j = 0
        while os.path.exists('./final_proposals/'+ str(j) + '.prop'):
            j += 1
        i = j + i
    propfilename = './final_proposals/'+ str(i) + '.prop'
    propfilename = './final_proposals/'+ str(i) + '.prop'
...
def read_final():
    """Read the database constructed by calls to
    :py:func:`.save_final_proposal` (i.e. from file
    "./final_proposals/database.info").
    Return a list with dicts of the proposal parameters in the database.
