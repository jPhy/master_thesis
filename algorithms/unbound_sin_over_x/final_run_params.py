...
dim = 10,
method = 'VB_pripos', # the algorithm to be executed, known are ['original', 'PMC_pripos', 'VB_pripos', 'VB_cornuet', 'VB_cornuet_corrected', 'weights_only']
first_proposal_index = 'good_variational', # the proposal to be taken from database, can be int or ['good, average' _ 'original, variational']
max_sampling_steps = 20, # the maximum number of sampling runs
N_c = 1000, # number of samples per component
max_vb_steps = 1000, # the maximum number of steps VB is allowed to run in each sampling (only if method = 'VB_...')
...
min_steps = 0, # the minimum number of proposal update steps
)
