...
N_MCMC = 5*10**4, # number of steps per chain
N_adapt = 500, # adapt the Markov chains every N_adapt steps (not during burn-in)

N_thin = 25, # use only every `N_thin`-th sample for the variational bayes (1 means all)

critical_r = 2., # the critical R-value for chain grouping (only needed if vb_initialization = 'long_patches')
K_g = 15, # number of components per chain_group (only neede for vb_initialization 'long_patches')
