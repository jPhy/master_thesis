...
\slide{
    ...
    \only<2->{ \begin{center} \vspace{3mm} \huge \textcolor{red}{we do not have the data $\mathcal{D}$ } \end{center} }

    \only<3->{\vspace{3mm} experimentalists measure relevant observables $\boldsymbol{O}$:

    $$ P(\mathcal{D}|\boldsymbol{\theta}, \mathrm{M}) = P(\mathcal{D} | \boldsymbol{O} (\boldsymbol{\theta} , \mathrm{M} )) $$

    \only<4->{experimentalists' publications provide $ P(\mathcal{D} | \boldsymbol{O}) $ \newline}
    \only<5->{theory provides $ \boldsymbol{O}(\boldsymbol{\theta} , \mathrm{M}) $ }
