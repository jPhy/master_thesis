...
\slide{
    ...
    \only<2->{
        ...
        $$ p(x_n | \boldsymbol{\theta}) = \sum_{k=1}^K \pi_k \gauss(x_n | \mu_k, \Sigma_k) $$
    ...
    \column{0.45\textwidth}
        ...
        \only<5->{$$ p(z_{nk}=1) = \pi_k = \prod ^K _{k=1} \pi_k^{z_{nk}} $$}

    \column{0.45\textwidth}

        \only<6->{$$ p(x_n | \boldsymbol{\theta}) = \sum_{\boldsymbol{Z}} p(x_n | \boldsymbol{Z} , \boldsymbol{\theta}) p(\boldsymbol{Z}) $$}
...
\slide{
    ...
    \only<2->{without proof} \only<1>{\vspace{4.8mm}}
        ...
        \[ \text{minimize} ~ \KLPp \uncover<2->{\Leftrightarrow \text{maximize} ~ \log p(\boldsymbol{X}) \equiv \prod^{N}_{n = 1} p(x_n)} \]
    ...
    \only<5->{\huge \textcolor{red}{\vspace{-11.5mm} \begin{center} assumption $q(\boldsymbol{Z} , \boldsymbol{\theta}) = q(\boldsymbol{Z})q(\boldsymbol{\theta})$ \newline $\Rightarrow$ semianalytical solution \end{center}}}
...
% TODO: x means done; o means to be done
%
%       x explain approach: \theta = {nuisance, Wilson coeff.}; split of P(data, observable(theta,M))
%       x present the 2d marginals as in thesis (only first figure)
...
%       x show subleading QCDF
%       o => conclusion: need better theoretical control of FF and subleading 1 / m_b to see significant
...
\slide[c]{ %TODO: desloppify
    ...
    \begin{itemize} %TODO: citations
        ...
        \begin{itemize}
            ...
            \item CDF 2012 ({\url{http://www-cdf.fnal.gov/physics/new/bottom/120628.blessed-b2smumu_96/public_b2smumu.pdf}})
            % angular observables $A_{FB}, F_H$,  LHCb 2014 ({\href{http://arXiv.org/abs/1403.8045}{arXiv:1403.8045}}) % TODO: write angular formula
        \end{itemize}
        \item $\mathcal{B}(B_s\rightarrow\mu^+\mu^-)$
        \begin{itemize}
    ...
    \begin{itemize}
        \item BtoKmumu angular analysis (LHCb 2014) %TODO: --> cite paper on arXiv
        \item BsTOmumu branching fraction (LHCb+CMS 2014) % TODO: --> cite paper on arXiv (experimental citation in methodology subsection)
...
%TODO: conclusion:
% we put updated, model-independent constraints on the scanned Wilson coefficients --> name them: C_S+prime, C_P+prime, C_T, CT5
% need better theoretical control to distinguish between yet unknown corrections and new physics
% the standard model can explain the latest LHCb data within the uncertainties
