...
from multimodal import LogTarget; log_target = LogTarget(dim)
...
plt.figure(); plt.hexbin(stacked_data[:,0], stacked_data[:,1], cmap='gray_r', extent=(-6, 6, -6, 6))
plt.xlabel('$x_1$')
plt.ylabel('$x_2$')
plt.savefig('plain_mc_data_1250.png') # not .svg because there too many hexagon objects --> rendering takes too long
...
plt.xlabel('$x_1$')
plt.ylabel('$x_2$')
plt.savefig('first_prop_pmc_1250.pdf')
print 'PMC done'
...
plt.xlabel('$x_1$')
plt.ylabel('$x_2$')
plt.savefig('first_prop_vb_1250.pdf')
...
plt.xlabel('$x_1$')
plt.ylabel('$x_2$')
plt.savefig('first_prop_hc_1250.pdf')
